...
One-Sided-Grid-Variant-Strategy-Annotated
...
This grid strategy allows the trading direction to be customized.

Buy first, then sell:
The strategy starts from the initial price and places buy orders downward at intervals defined by the "Price spacing" parameter. Each order uses the "Single order size" setting, and the strategy places the configured total number of buy orders. After any buy order is filled, the strategy places a sell order at the filled buy price plus the value of the "Price difference (quote currency)" parameter. Once that sell order is completed, the corresponding buy order is placed again at the original grid level.

Sell first, then buy:
The logic is exactly the reverse.

The main risk of the strategy is a one-sided market in which price moves beyond the grid range.

The grid also includes automatic stop-loss and auto-shift functionality.

// Xiaoxiaomeng / Menghui
// Summary article - FMZ Inventor Quant video and text tutorials: https://www.fmz.com/bbs-topic/334
// One-Sided Grid Variant Strategy (annotated version): https://www.fmz.com/bbs-topic/1069
...
|FirstPriceAuto|true|Auto initial price|
|FirstPrice|100|Initial price|
|AllNum|10|Total number of orders|
|PriceGrid|true|Price spacing|
|PriceDiff|2|Price difference (quote currency)|
|AmountType|0|Order size: Same buy/sell size|Custom size|
|AmountOnce|0.1|Single order size|
|BAmountOnce|0.1|Buy order size|
|SAmountOnce|0.1|Sell order size|
|AmountCoefficient|*1|Size multiplier|
|AmountDot|3|Maximum decimal places for size|
|EnableProtectDiff|false|Enable spread protection|
|ProtectDiff|20|Entry spread protection|
|CancelAllWS|true|Cancel all pending orders on stop|
|CheckInterval|2000|Polling interval|
|Interval|1300|Retry interval after failure|
|RestoreProfit|false|Restore previous profit|
|LastProfit|false|Previous profit|
|ProfitAsOrg|false|Include previous profit in average price|
|EnableAccountCheck|true|Enable balance check|
|EnableStopLoss|false|Enable stop-loss|
|StopLoss|100|Maximum floating loss (quote currency)|
|StopLossMode|0|Action after stop-loss: Close and exit|Close and restart the grid|
|EnableStopWin|false|Enable take-profit|
|StopWin|120|Maximum floating profit (quote currency)|
|StopWinMode|0|Action after take-profit: Close and exit|Close and restart the grid|
|AutoMove|false|Auto shift|
|MaxDistance|20|Maximum distance (quote currency)|
|MaxIdle|7200|Maximum idle time (seconds)|
|EnableDynamic|false|Enable dynamic pending orders|
|DynamicMax|30|Order invalidation distance (quote currency)|
|ResetData|true|Clear all data on startup|
|Precision|5|Price decimal precision|
|XPrecision|5|Order size decimal precision|
...
|----|----|----|
|CloseGrid|__button__|Stop and rebalance to the initial state|
