/*
策略出处: https://www.botvs.com/strategy/21487
策略名称: 从第三方网站获取K线 (0804更新)+ bitstamp+okcoin现货
策略作者: 青铜战国级机器人
策略描述:
...
$.AltRecords = function(exchange, timeframe, size, includeLastBar) {
    ...
    else if (exchange.GetName().indexOf('CHBTC') != -1 && exchange.GetCurrency().indexOf('BTC') != -1) {
        symbol = "chbtcbtccny";
    }
    else if (exchange.GetName().indexOf('Bitstamp') != -1 && exchange.GetCurrency().indexOf('BTC') != -1) {
        symbol = "bitstampbtcusd";
    }
    else if (exchange.GetName().indexOf('Futures_OKCoin') != -1 && exchange.GetCurrency().indexOf('BTC') != -1) {
        symbol = "okcoinbtcusdfuture";
    }
    else if (exchange.GetName().indexOf('OKCoin_EN') != -1 && exchange.GetCurrency().indexOf('BTC') != -1) {
        symbol = "okcoinbtcusd";
    }
    else if (exchange.GetName().indexOf('OKCoin') != -1 && exchange.GetCurrency().indexOf('BTC') != -1) {
        symbol = "okcoincnbtccny";
    }
    else if (exchange.GetName().indexOf('Huobi') != -1 && exchange.GetCurrency().indexOf('BTC') != -1) {
        symbol = "huobibtccny";
    }
    else if (exchange.GetName().indexOf('BTCE') != -1 && exchange.GetCurrency().indexOf('BTC') != -1) {
        symbol = "btcebtcusd";
    }
...
function main() {
    ...
    rec = $.AltRecords(exchange, "1hour", 1000000); // 获取4小时K线, 100条, 含最后一条Bar
    Log(rec);
    if (rec) Log(rec.length, rec[rec.length-1]);
}
