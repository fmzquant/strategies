...
Single-Point-Sniper-High-Frequency-Averaging-Auto-Reversal-Recovery-Algorithm-V12
...
Open an initial position and place a take-profit order at a fixed target. If the position cannot be closed and price moves beyond the stop threshold, the strategy adds to the position to lower the average cost until it can exit in profit. If capital or inventory is exhausted, it can automatically reverse direction: a failed short flips to long, and a failed long flips to short. Auto-reversal can also be disabled so the program simply waits for a recovery exit.

For example, if a long position is opened with an average cost of 10 and the profit target is 0.5, the strategy places a sell order at 10.5. If price falls, it adds near the current market price, recalculates the average cost, and reposts the target-profit order. If that still fails, it keeps averaging down until it can exit; if the position remains trapped, it reverses into a short.

### Auto-Reversal Mechanism

If a long position closes profitably, the strategy automatically opens another long. If the long becomes trapped, it reverses into a short using the inventory on hand. After a profitable short exit, it continues opening shorts until it can no longer keep adding, then flips back to long. As long as the previous cycle ended in profit, the strategy keeps the same opening direction.

The strategy automatically recalculates both the required add-on size and the new profit target after each position adjustment.

### Notes

This strategy includes a complete state-recovery mechanism and can be used for live trading or study.
The only way it can be “100% profitable” is if enough capital is reserved for continued averaging.
If funds are insufficient, enabling auto-reversal may result in floating profit and loss.
If capital is sufficient, auto-reversal is unnecessary and the program can keep averaging until recovery.
By tuning the parameters, the strategy can be adapted into a high-frequency order-book style strategy.
It is not suitable for futures, where this approach can easily lead to liquidation, so it should only be used in spot trading.

### Open-Source Purpose

The goal of publishing this code is to attract more people into the quantitative trading community instead of working in isolation.

V1.1 fixes a bug on OKCoin where 0.0001 frozen coins could cause the program to stall.

For a more detailed explanation, see: https://www.fmz.com/#!/bbs-topic/38
...
|----|----|----|
|OpType|0|Initial Opening Direction: Long|Short|
|OpAmount|0.1|Opening Position Size|
|OpMode|0|Opening Order Type: Taker|Maker|
|MaxSpace|0.5|Maker Order Invalidation Distance|
|SlidePrice|0.1|Order Sliding Price (CNY)|
|MaxAmount|0.3|Maximum Single Opening Order Size|
|AddGoal|true|Add-On Interval (CNY)|
|AddLine|0.8|Target Average Price After Add-On (CNY)|
|ProfitGoal|0.5|Exit Profit Target (CNY)|
...
|RestoreAmount|false|Position Size|
|RestoreProfit|false|Previous Profit|
|SaveLocal|false|Save Local Log|
|AutoReverse|true|Auto Reverse|
|MinStock|0.01|Minimum Tradable Coin Amount|
