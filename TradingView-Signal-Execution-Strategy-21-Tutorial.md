...
Added reverse-position commands.
...
|FMZ_AccessKey|Note: this is the FMZ platform AccessKey, not the exchange API key|FMZ platform AccessKey|
|FMZ_SecretKey|Note: this is the FMZ platform SecretKey, not the exchange API key|FMZ platform SecretKey|
|maxBuffSignalRowDisplay|Maximum number of signal rows shown in the status bar; setting 20 displays the latest 20 signal records|Maximum displayed signal rows|
...
|----|----|----|
|TestSignal|Used only to simulate a webhook request sent by TradingView|Test signal|
|evalCode|Execute JavaScript code directly; can be used for testing or switching to paper trading|Execute JavaScript code|
