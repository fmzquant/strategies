'''
策略出处: https://www.botvs.com/strategy/21890
策略名称: 均线策略改版（python版） (复制)
策略作者: sy10001
策略描述:
...
--------------  -----  ------
FastPeriod      3      入市快线周期
SlowPeriod      20     入市慢线周期
EnterPeriod     3      入市观察期
ExitFastPeriod  3      离市快线周期
ExitSlowPeriod  16     离市慢线周期
    ...
    while True:
        if state == STATE_IDLE :
            ...
            if abs(n) >= EnterPeriod :
                ...
                Dict = ext.Buy(opAmount) if n > -0.04 else ext.Sell(opAmount)
                if Dict :
                    opAmount = Dict['amount']
                    state = PD_LONG if n >-0.04 else PD_SHORT
