...
Futures-spot hedging is an arbitrage approach that captures the spread between futures and spot prices. Because futures converge toward the spot price at settlement, a spread can be locked in by shorting the futures contract and buying spot, or by going long futures and selling spot.

For example, if BTC spot is $20,000 and BTC futures are $25,000, buying 1 BTC spot while shorting 1 BTC futures can lock in a $5,000 profit by settlement.

The risk of futures-spot hedging is relatively low. Under typical OKEX market conditions, it may achieve roughly 40% to 50% annualized returns, and returns can be even higher during extreme bull or bear markets.

Strategy logic: this strategy automatically detects the spread between OKEX spot and futures markets. When the spread reaches the target profit threshold, it opens an equal-sized hedge to capture the basis.

Strategy features:
- Supports all OKEX futures products, including BTC, EOS, BCH, and ETH
- Supports customizable leverage and contract types, such as this week and next week contracts
- Supports customizable profit targets, such as a 40% annualized return goal
- Provides detailed reporting, including strategy status, trade history, and profit tracking
- Fully automates the hedge without requiring manual intervention

Main risk: exchange default or platform failure risk.

Supported platform: BotVS/FMZ

Detailed parameter guide: https://www.pcclean.io/7w0e
> Source (javascript)
