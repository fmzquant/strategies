...
``` javascript
//保证金市场风险巨大，可能随时面临100%损失或有不明bug100%损失，概不负责。本策略使用的杠杆相对不大，可放心体验
//实盘地址：https://api.fmex.com 
...
function underElephant (ticker) {
    ...
    if (bestBidAmount > 40000 && bestBidAmount > bestAskAmount * 2) {
        //如果，买一档的单量＞X万，且买一的单量大于卖一的单量的N倍，挂
        //等待三秒，撤单。重新检测，重新挂。
       // order = createOrderPrice({symbol: symbol,type: "LIMIT",direction: "LONG",post_only: true,price: buyPrice - 0.5,quantity: sp_perAmount * 3})
        ...
        //如果，卖一档的单量＞X万，且卖一的单量大于买一的单量的N倍，挂
        //等待三秒，撤单。重新检测，重新挂。
...
https://www.fmz.com/strategy/219929
