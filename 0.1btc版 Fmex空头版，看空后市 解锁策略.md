...
|minPrice|8000|区间最低价|
|g_maxHoldingLong|2500|多单最大持仓量|
|g_maxHoldingShort|2900|空单最大持仓量|
|sp_baseAmountShort|2200|仓位控制：空仓过多，转向成交多头的限制仓位|
|sp_baseAmountLong|true|仓位控制：多仓过多，转向成交空头的限制仓位|
|sp_perAmount|120|市商排序：单量。(深度排序单量=市商单量*3)(正常多头单量=市商单量*0.6)|
|Interval|3|Intervel0轮询时间（默认参数即可）|
|mainIntervel|true|Intervel市商轮询时间（默认参数即可）|
        ...
        if (it.symbol == symbol) {
            if (it.quantity) {
                ...
                if (it.quantity > g_maxHoldingShort && it.direction.toUpperCase() == 'SHORT') {
                    data = createOrder({symbol: symbol,type: "MARKET",direction: 'LONG',quantity: sp_perAmount 
                    })
                    Log("SHORT超最大仓位,减仓");
                }
            }
    ...
    res.results.forEach(function(it) {
        if (it.quantity) {
            ...
            if (it.quantity > sp_baseAmountLong && it.direction.toUpperCase() == 'LONG') {//多仓过多，改变挂单档位
                ...
                for (index = 0; index < 5; index++) {
                    order = createOrderPrice({symbol: symbol, type: "LIMIT", direction: "LONG",  post_only: true,  price: lastPrice - 2 - 0.5 * index,   quantity: sp_perAmount                   })
...
https://www.fmz.com/strategy/218140
