...
Keltner-Channel-Breakout-Stop-Loss-and-10-Percent-Profit-Long-Term-Hold-Strategy-v23-dev-Multi-Timeframe
...
This is a long-only BTC spot strategy that starts with 1,000 USDT in cash.
It checks once per hour whether price has broken above the Keltner Channel, and opens a long position when a breakout occurs.

Exit logic:
1. If the position loses 6%, stop out immediately.
2. If price drops below the MA line, sell immediately.
3. If profit reaches 10%, that gain becomes a cushion and the strategy continues holding long. If price then falls 10% over the past 24 hours, it is treated as a sudden event and the position is stopped out.

The author reports using this strategy in live trading, with backtest results that look reasonably good.
In essence, the idea is simple: keep losses small and let profits run.
