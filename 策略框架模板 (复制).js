/*
策略出处: https://www.botvs.com/strategy/33431
策略名称: 策略框架模板 (复制)
策略作者: zhongguo888
策略描述:
...
function OpenLong(task){
    // Log("Buy", task.OpenLongAmount, task.Exchange.GetName(), task.Indication); // ceshi 
...
function CalProfit(task){
    ...
    LogProfit(SumProfit, task.Name + "-" + task.Currency + "-" + task.Label + "- 本次：" + (task.Profit - task.preProfit));
}
...
$.ToTable = function(){
    ...
    _.each(Tasks, function(task){
        ...
        // 处理自定义
        // OpenLongAmount OpenShortAmount AddAmount StopLossAmount STATE minStock initAccount Currency Name Label nowAccount 
        // lastAccount lastTASKSTATE Profit floatProfit lastPrice preProfit NowPositionInfo
        for(var key in task){
    ...
    // var table = {type: "table", title: "", cols: [], row: []};
};
